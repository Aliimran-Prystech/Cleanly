...
const Booking = ({ isLoggedIn = false, handleLogout }) => {
  ...
  useEffect(() => {
    const loadServices = async () => {
      try {
        ...
        const legacyAddons = Array.isArray(savedConfig.addons) ? savedConfig.addons : [];
        setServiceConfig({
          ...
          cleaningTypes: {
            ...
            standard: { ...defaultServiceConfig.cleaningTypes.standard, addons: legacyAddons, ...(savedConfig.cleaningTypes?.standard || {}) },
            deep: { ...defaultServiceConfig.cleaningTypes.deep, addons: legacyAddons, ...(savedConfig.cleaningTypes?.deep || {}) },
            moveInOut: { ...defaultServiceConfig.cleaningTypes.moveInOut, addons: legacyAddons, ...(savedConfig.cleaningTypes?.moveInOut || {}) },
          },
          frequencyDiscounts: {
  ...
  const calculateTotal = () => {
    ...
    const rates = ratesByType[cleaningTypeValues[cleaningType]] || ratesByType.Standard;
    ...
    return Number((total - total * ((discounts[frequency] || 0) / 100)).toFixed(2));
  };
  ...
  const resetBookingForm = () => {
    ...
    setPersonalDetails({ name: "", email: "", phone: "", address: "", zipCode: "" });
  ...
  const handleBookingSubmit = async () => {
    ...
    try {
      ...
      setSuccessMessage(`Booking created. Total: $${Number(response.data?.booking?.totalCost || calculateTotal()).toFixed(2)}`);
      resetBookingForm();
    } catch (bookingError) {
      console.error("Booking submission failed:", bookingError);
      setError(bookingError.response?.data?.message || "Unable to create booking. Please try again.");
    } finally {
      setSubmitting(false);
    }
  };
          ...
          <div className="booking-card">
            ...
            <div className="booking-card__body">
              ...
              <div className="booking-field">
                ...
                <input type="date" min={getToday()} value={bookingDate} onChange={(e) => setBookingDate(e.target.value)} />
              </div>
              ...
              <div className="personal-details">
                ...
                <input type="text" placeholder="Full Name" value={personalDetails.name} onChange={(e) => updatePersonalDetail("name", e.target.value)} required />

                <div className="personal-details__row">
                  <input type="email" placeholder="Email Address" value={personalDetails.email} onChange={(e) => updatePersonalDetail("email", e.target.value)} required />

                  <input type="tel" placeholder="Phone Number" value={personalDetails.phone} onChange={(e) => updatePersonalDetail("phone", e.target.value)} required />
                </div>
                ...
                <div className="personal-details__row">
                  ...
                  <input type="text" placeholder="Zip" className="zip-input" value={personalDetails.zipCode} onChange={(e) => updatePersonalDetail("zipCode", e.target.value)} required />
                </div>
                ...
                {successMessage && <p className="booking-message booking-message--success">{successMessage}</p>}

                <button type="button" className="complete-booking-btn" onClick={handleBookingSubmit} disabled={submitting || loadingServices}>
                  {submitting ? "Submitting..." : loadingServices ? "Loading pricing..." : "🔒 Complete Booking via Secure Server"}
                </button>
              </div>
            </div>
          </div>
